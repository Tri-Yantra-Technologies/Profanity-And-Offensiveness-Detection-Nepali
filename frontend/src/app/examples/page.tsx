                        ...
                        {EXAMPLES.map((ex, i) => (
                            ...
                            >
                                <div className="p-6 pb-4 border-b border-border/50">
                                    <div className={`inline-block px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${getCategoryColor(ex.category)} text-white mb-3`}>
                                        {ex.category}
                                    </div>
                                    <p className="text-lg font-medium leading-relaxed nepali-text">"{ex.text}"</p>
                                    <p className="text-sm text-muted-foreground mt-1">{ex.romanized}</p>
                                    {ex.note && (
                                ...
                                <div className="p-6 pt-4 space-y-3">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-muted-foreground font-mono">{(ex.profanity.confidence * 100).toFixed(0)}%</span>
                                            <Badge variant={ex.profanity.label === "Profane" ? "destructive" : "default"} className={ex.profanity.label !== "Profane" ? 'bg-green-500/20 text-green-400 border-green-500/30 text-xs' : 'text-xs'}>
                                                {ex.profanity.label}
                                            </Badge>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-muted-foreground font-mono">{(ex.offensiveness.confidence * 100).toFixed(0)}%</span>
                                            <Badge variant={ex.offensiveness.label === "Offensive" ? "destructive" : "default"} className={ex.offensiveness.label !== "Offensive" ? 'bg-green-500/20 text-green-400 border-green-500/30 text-xs' : 'text-xs'}>
